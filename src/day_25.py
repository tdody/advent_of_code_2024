...
class Lock:
    ...
    def __repr__(self):
        return ",".join([str(p) for p in self.pins])
...
class Key:
    ...
    def __init__(self, teeth: list[int]):
        self.teeth = teeth

    def flip(self) -> list[int]:
        max_height = max(self.teeth)

        return [max_height - x for x in self.teeth]

    def __repr__(self):
        return ",".join([str(t) for t in self.teeth])
        ...
        for i in range(len(data[0])):
            for n in range(len(data)):
                if data[n][i] == "#":
                    teeth.append(6 - n)
                    break
        logger.debug(f"Teeth: {teeth}")
        logger.debug("-------")
        return cls(teeth)


def fit_key_in_lock(key: Key, lock: Lock):
    logger.debug(f"K: {key}")
    logger.debug(f"L: {lock}")

    for i in range(len(key.teeth)):
        if key.teeth[i] + lock.pins[i] > 5:
            logger.debug(f"Lock {lock} and key {key}: overlap on column {i+1}.")
            return False
    logger.debug(f"Lock {lock} and key {key}: all columns fit!")
    return True


def _is_lock(data: list[str]) -> bool:
    return all([c == "#" for c in data[0]])
...
def read_file(file_path: str) -> tuple[list[Key], list[Lock]]:
    ...
    i = 0
    while i < len(data):
        if _is_lock(data[i : i + 7]):
            locks.append(Lock.from_data(data[i : i + 7]))
            i += 7
        else:
            keys.append(Key.from_data(data[i : i + 7]))
            i += 7
    return keys, locks
