...
def paths(keymap: str) -> dict[tuple[str, str], list[str]]:
    """
    Generate all possible paths between two keys on a keypad.

    >>> paths("1A")
    {('1', '1'): [''], ('1', 'A'): ['A'], ('A', '1'): ['^A'], ('A', 'A'): ['']}
    """
    # generate the locations of each key on the keypad
    locations = {
        c: (x, y)
    ...
    # store the possible paths between each pair of keys
    pathmap = {}

    # generate the possible paths between each pair of keys
    for a, b in product((c for c in keymap if c != " "), repeat=2):
        ...
        # get the coordinates of the two keys
...
def part_1(file_path: str) -> int:
    ...
    for code in codes:
        ...
        complexity += presses(code, n_robots + 2) * code_value
    return complexity
