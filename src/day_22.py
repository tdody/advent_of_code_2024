...
def _mix(value: int, secret: int) -> int:
    """
...
@cache
def simulate_one(secret: int) -> int:
    """
    Simulate the creation of a new secret number.
    """

    secret = _mix(secret * 64, secret)
    secret = _prune(secret)

    secret = _mix(secret // 32, secret)
    secret = _prune(secret)

    secret = _mix(secret * 2048, secret)
    secret = _prune(secret)
...
class Sequence:
    ...
    def __init__(self, secret: int):
        ...
        self._prices = {}
        self._price_differences: list[int] = []
    ...
    def _update_differences(self) -> None:
        ...
        if (
            len(self._price_differences) == 4
            and tuple(self._price_differences) not in self._prices
        ):
    ...
    def compute_step(self) -> None:
        ...
        self._secret = simulate_one(self._secret)
        self._price = self._get_price()
...
def read_code(file_path: str) -> list[int]:
    """
    Read the input file and return the list of secret numbers.
    """
...
def part_1(file_path: str) -> int:
    ...
    secret_numbers = read_code(file_path)
    for _ in range(2000):
        secret_numbers = [simulate_one(secret) for secret in secret_numbers]
...
def part_2(file_path: str) -> int:
    ...
    for secret_number in tqdm(secret_numbers):
        sequence = Sequence(secret_number)
        for _ in range(2000):
            sequence.compute_step()
        for s, p in sequence.get_prices().items():
